...
const CAT_CONFIG = {
  rocher:     {label:'Rocher Wholesaler',  emoji:'🍫'},
  gallery:    {label:'Gallery Wholesaler', emoji:'🌟'},
  raffaello:  {label:'Raffaello Wholesaler',emoji:'🥥'},
  rondnoir:   {label:'Rondnoir Wholesaler',emoji:'🍪'},
  hazelnut:   {label:'Hazelnut Wholesaler',emoji:'🌰'},
  assortment: {label:'Assortments Wholesaler',emoji:'🎁'},
};

export const DistSetup = () => {
    ...
    const fetchDBCategories = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          const config = {};
          data.forEach(item => {
            config[item.code] = { label: item.label + ' Dist.', emoji: item.emoji };
          });
  ...
  const fetchLoc = () => {
    ...
    setTimeout(() => {
      ...
      showToast('📍 HQ Location detected!');
    }, 1000);
  };

  const handleNext = async () => {
    if (!shopName) { showToast('⚠️ Enter company name'); return; }
  ...
  return (
    <div className="screen active">
      <div className="ghead" style={{ padding: '.875rem 1.1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem' }}>
            ...
            <div><h2 style={{ fontSize: '1.05rem', fontWeight: 800 }}>Distributor Setup</h2><p style={{ fontSize: '.65rem', color: 'var(--t2)' }}>Step 1 of 4</p></div>
          </div>
      ...
      <div className="scroller" style={{ padding: '1.25rem' }}>
        <p style={{ fontSize: '.85rem', color: 'var(--t2)', marginBottom: '1.4rem', lineHeight: 1.5 }}>Set up your wholesale operation to start receiving orders from retailers across the CounterOS network.</p>
        
        <div className="au d1" style={{ display: 'grid', gap: '1.2rem', marginBottom: '1.5rem' }}>
          <Input label="Company / Trade Name" placeholder="e.g. Sharma Wholesale" value={shopName} onChange={e=>setShopName(e.target.value)} />
          <Input label="Owner / Manager Name" placeholder="e.g. Sharma Ji" value={ownerName} onChange={e=>setOwnerName(e.target.value)} />
          <Input label="GST Number" placeholder="e.g. 23XXXXX0000X1Z5" value={gst} onChange={e=>setGst(e.target.value)} style={{textTransform:'uppercase'}} />
        </div>

        <div className="au d2" style={{ marginBottom: '1.5rem' }}>
          <label className="ilabel">Primary Supply Category</label>
        ...
        <div className="au d3" style={{ marginBottom: '2rem' }}>
          <label className="ilabel">Headquarters Location</label>
          <div 
          ...
          >
            {locState.status === 'idle' && (
              <>
                ...
                <p style={{ fontSize: '.85rem', fontWeight: 700, color: 'var(--g4)', marginBottom: '.2rem' }}>Detect HQ Location</p>
          ...
          {locState.status !== 'detected' && (
            <div style={{ marginTop: '.8rem' }}>
              <Input placeholder="Or enter manual address" value={locState.name || ''} onChange={e => setLocState(prev => ({...prev, name: e.target.value}))} />
            </div>
          )}
        </div>

        <div className="au d4" style={{ paddingBottom: '1.5rem' }}>
          <Button onClick={handleNext}>Initialize Distributor Dashboard <span className="material-symbols-outlined" style={{ fontSize: '1.1rem' }}>arrow_forward</span></Button>
        </div>
      </div>
    </div>
  );
